...
type ResponseType = InferResponseType<typeof client.api.projects[':projectId']['tasks'][':taskId']['$patch'], 200>;
type RequestType = InferRequestType<typeof client.api.projects[':projectId']['tasks'][':taskId']['$patch']>;
...
export const useUpdateTask = ({ projectId }: { projectId: string }) => {
    ...
    const mutation = useMutation<ResponseType, Error, RequestType>({
        mutationFn: async ({ param, json }) => {
            const response = await client.api.projects[':projectId']['tasks'][':taskId'].$patch({
                param,
                json,
            });
